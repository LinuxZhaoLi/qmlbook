...
void Shorty::shootFull(const QString &name)       // 截屏
{
...
void Shorty::shoot(const QString &name)     // 截屏缩放
{
    qDebug() << "Shoot: " << name;
    QImage image = _view->grabWindow();
...
void Shorty::sendKeyPress(int key)        // 按键事件
{
    qDebug() << "Sending keypress for" << key;
    // exec()的实质是不停调用processEvent()函数从队列中获取事件，并处理，
    //然后删除，postEvent的作用就是发送一个事件到此队列中
...
void Shorty::sendMouseClick(int x, int y)   // 转发鼠标事件
{
